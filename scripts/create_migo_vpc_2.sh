#!/bin/bash
# create_migo_vpc_2.sh <VPC_NAME> <HOST_INTERFACE>
VPC_NAME="$1"
HOST_IF="$2"
LOGDIR="logs"
...
log "START create_migo_vpc_2: $VPC_NAME on host-if $HOST_IF"
...
log "Attaching veth to namespaces and bridge"
...
iptables -t nat -A POSTROUTING -s 10.1.0.0/16 -o "$HOST_IF" -j MASQUERADE 2>&1 | tee -a "$LOGFILE" || log "Note: iptables rule may already exist"

log "END create_migo_vpc_2"
