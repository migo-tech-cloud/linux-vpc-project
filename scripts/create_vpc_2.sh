#!/bin/bash
# Usage: ./create_vpc_2.sh <VPC_NAME> <HOST_IFACE>
VPC_NAME=$1
HOST_IFACE=$2
...
# Assign IPs
sudo ip netns exec $PUB_NS ip addr add 10.1.1.2/16 dev veth2-pub
sudo ip netns exec $PRIV_NS ip addr add 10.1.2.2/16 dev veth2-priv

# Bring up interfaces inside namespaces
sudo ip netns exec $PUB_NS ip link set veth2-pub up
sudo ip netns exec $PRIV_NS ip link set veth2-priv up
...
sudo ip netns exec $PUB_NS ip route add default via 10.1.0.1
sudo iptables -t nat -A POSTROUTING -s 10.1.0.0/16 -o $HOST_IFACE -j MASQUERADE
